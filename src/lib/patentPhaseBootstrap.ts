 ...
 * - Plan `returned` or still `pending` → plan tab (step 1).
 * - Plan `approved` → checklist (step 2), unless checklist already approved → step 3.
 */
export function serverSuggestedPatentPhase(params: {
  ...
  if (params.planStatus === 'returned' || params.planStatus === 'none' || params.planStatus === 'pending') return 1
